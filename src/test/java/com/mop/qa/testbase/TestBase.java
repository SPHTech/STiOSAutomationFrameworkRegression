...
//import com.mop.qa.pageobject.BeritaHarian_Android;
//import com.mop.qa.pageobject.BeritaHarian_IOS;
...
public class TestBase {
	...
	@BeforeTest
	public void startTestReport(ITestContext ctx)throws IOException 
	{
		try 
		{
	...
	/*
	 * @BeforeMethod public void beforeMethod(Method method,ITestContext ctx) {
	 * 
	 * try{ String browser = ctx.getCurrentXmlTest().getParameter("browser");
	 * toolName = ctx.getCurrentXmlTest().getParameter("toolName"); udid =
	 * ctx.getCurrentXmlTest().getParameter("udid"); platform_name =
	 * ctx.getCurrentXmlTest().getParameter("platformName"); appType =
	 * ctx.getCurrentXmlTest().getParameter("appType"); String exeType =
	 * ctx.getCurrentXmlTest().getParameter("ExecutionType"); String localityType =
	 * ctx.getCurrentXmlTest().getParameter("Locality"); currentTest =
	 * ctx.getCurrentXmlTest().getParameter("testname"); //String platform_name =
	 * ""; RemoteUrl = ctx.getCurrentXmlTest().getParameter("RemoteUrl"); String
	 * startURL = rds.getValue("DATA", currentTest, "URL");
	 * ExtentUtility.startTest(method.getName());
	 * System.out.println(method.getName()); initiateApp(localityType, appium_port,
	 * browser, RemoteUrl, platform_name, toolName,appType,startURL); }
...
//		ExtentUtility.getReporter().flush(); 
	...
	/*
	 * public TTA_Admin_Quote selectTools() { TTA_Admin_Quote TTAA;
	 * if(toolName.equalsIgnoreCase("Selenium")) {
	 * 
	 * TTAA = new TTA_Admin_Quote(remoteDriver); } else { TTAA = new
	 * TTA_Admin_Quote(appiumDriver); } return TTAA; }
	 */
	
	public StraitsTimes_IOS toSelectTools() {
		
		StraitsTimes_IOS STIOS;
		if(toolName.equalsIgnoreCase("Selenium")) {
			
			STIOS = new StraitsTimes_IOS(remoteDriver);
		}
		else {
			STIOS = new StraitsTimes_IOS(appiumDriver);
		}
		return STIOS;
	}
	
	
	public StraitsTimes_IOS selectTools2() {
		StraitsTimes_IOS STO;
		if(toolName.equalsIgnoreCase("Selenium")) {



		STO = new StraitsTimes_IOS(remoteDriver);
		}
		else {
		STO = new StraitsTimes_IOS(appiumDriver);
		}
		return STO;
		}
	
	public StraitTimesIOS selectTools4() {
		
		StraitTimesIOS STIOS;
		
		if(toolName.equalsIgnoreCase("Selenium")) {
		STIOS = new StraitTimesIOS(remoteDriver);
		}
		else {
		STIOS = new StraitTimesIOS(appiumDriver);
		}
		return STIOS;
		}
	
	public StraitTimes_ios_objects selectTools5() {
		StraitTimes_ios_objects ST_ios;
		if(toolName.equalsIgnoreCase("Selenium")) {



		ST_ios = new StraitTimes_ios_objects(remoteDriver);
		}
		else {
		ST_ios = new StraitTimes_ios_objects(appiumDriver);
		}
		return ST_ios;
		}
	...
	/*
	 * public BeritaHarian_IOS to_SelectTools() { BeritaHarian_IOS STIOS;
	 * if(toolName.equalsIgnoreCase("Selenium")) {
	 * 
	 * STIOS = new BeritaHarian_IOS(remoteDriver); } else { STIOS = new
	 * BeritaHarian_IOS(appiumDriver); } return STIOS; }
	 * 
	 * public BeritaHarian_Android to__SelectTools() { BeritaHarian_Android STIOS;
	 * if(toolName.equalsIgnoreCase("Selenium")) {
	 * 
	 * STIOS = new BeritaHarian_Android(remoteDriver); } else { STIOS = new
	 * BeritaHarian_Android(appiumDriver); } return STIOS; }
	 */
//@AfterTest
	@AfterSuite
	...
	public void initiateApp(String localityType, String port, String browser, String RemoteUrl, String platform_name,
			String toolname, String appType, String startURL)
	{
		try 
		{
			...
			{
				PageBase pagebaseclass = new PageBase(appiumDriver);
				appiumDriver = pagebaseclass.onlylaunchingApp(udid, localityType, RemoteUrl, toolname, appType, port,
						platform_name,startURL);
			}
			else if (toolName.equalsIgnoreCase("selenium")) 
			{
